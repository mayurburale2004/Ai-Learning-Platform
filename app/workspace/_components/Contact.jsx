...
function Contact() {
  ...
  const onSubmit = async (event) => {
    ...
    formData.append("access_key", "5f167728-3d82-4830-aaf1-c9930a585bf9");
  ...
  return (
     ...
     id='contact' className='w-full px-[12%] py-10 scroll-mt-20 bg-[url("/footer-bg-color.png")] bg-no-repeat bg-center bg-size-[90%_auto]'>
        <motion.h4 initial={{opacity: 0, y: -20}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.5, delay:0.3}}
         className=' text-center mb-2 text-lg'>Connect with us</motion.h4>
        ...
        <motion.form initial={{opacity: 0}} whileInView={{opacity: 1}} transition={{duration: 0.5, delay:0.9}}
            ...
            <motion.button whileHover={{scale: 1.05}} transition={{duration: 0.3}}
             type='submit'
            className='py-3 px-8 w-max flex items-center justify-between gap-2 bg-black/80 text-white rounded-full mx-auto hover:bg-black duration-500 cursor-pointer'>Submit now <Image src={'/right-arrow-white.png'} width={20} height={20} alt='arrow' />
            </motion.button>
